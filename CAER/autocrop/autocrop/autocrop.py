...
class Cropper:
    ...
    * `fix_gamma`: `bool`, default=`True`
    ...
    def __init__(
        self, width=224, height=224, face_percent=50, padding=None, fix_gamma=True,
    ):
    ...
    def crop(self, path_or_array):
        ...
        mask[pos[0] : pos[1], pos[2] : pos[3]] = 0
        ...
        return bgr_to_rbg(image), bgr_to_rbg(image_masked)

    def failed_file(self, path_or_array):
        if isinstance(path_or_array, str):
            image = open_file(path_or_array)
        else:
            image = path_or_array
